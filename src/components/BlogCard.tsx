...
const BlogCard = ({ blog }: any) => {
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {blog?.length == 0 ? (
        <div>No blog posted yet.</div>
      ) : (
        <>
          {blog.slice(0, 3).map((post: any, index: number) => (
            <motion.article
              key={index}
              ...
              <Link href={`/blog/${post.slug}`}>
                ...
                >
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    {/* Title */}
                    <h3 className="font-display font-semibold text-xl text-foreground group-hover:text-[#E1B505] transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    {/* Read More */}
                    <div className="flex items-center gap-2 text-[#E1B505] text-sm font-medium pt-2">
